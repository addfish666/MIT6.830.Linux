...
//    int pageSize;
    ...
    /**
     * key是活跃事务tid，value就是当前事务的第一条record offset
     * 开始BEGIN日志的时候会往map里put一个tid和对应的offset进去
     * 在事务完成(COMMIT或ABORT)以后会删除这个map里对应的tid的记录
     * 所以这个map里实际上保存的是正在进行的事务的BeginOffset
     * */
